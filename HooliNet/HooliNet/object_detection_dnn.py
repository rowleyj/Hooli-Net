...
with open('HooliNet\HooliNet\classification_classes_ILSVRC2012.txt', 'r') as f:
   image_net_names = f.read().split('\n')
# final class names (just the first word of the many ImageNet names for one image)
class_names = [name.split(',')[0] for name in image_net_names]

#load the neural network model
model = cv2.dnn.readNet(model='HooliNet\HooliNet\DenseNet_121.caffemodel', config='HooliNet\HooliNet\DenseNet-121.prototxt', framework='Caffe')

# load the image from disk
img_file = "HooliNet\HooliNet\car4.jpg"
